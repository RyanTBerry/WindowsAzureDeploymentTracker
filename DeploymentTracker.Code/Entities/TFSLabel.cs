...
namespace DeploymentTracker.Services.Entities
{
    ...
    public class TFSLabel
    {
        ...
        {
            if (string.IsNullOrEmpty(this.LabelName) || this.LabelName.Length < 3)
            {
                throw new ArgumentException("Enter name search criteria. Should be atleast 3 characters.");
            }

            if (this.LabelName.Length >= 64 || this.LabelName.Contains(" "))
            {
                throw new ArgumentException("Invalid TFS label name.");
            }

            foreach (var item in this.LabelName.ToCharArray())
            {
